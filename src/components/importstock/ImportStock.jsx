...
const ImportStock = ({ products, setProducts }) => {
  ...
  const handleChange = (index, field, value, selectedProduct = null) => {
    ...
    if (field === 'productCode') {
      const product =
        selectedProduct || products.find((p) => p.productCode === value);
            ...
            <div className="form-row" key={index}>
              <Autocomplete
                ...
                onInputChange={(e, newInputValue) => {
                  handleChange(index, 'productCode', newInputValue);
                }}
                renderInput={(params) => (
                  <TextField
                    {...params}
              ...
              <input
                list="noteOptions"
                placeholder="Nhập ảo / Nhập thực / Tuỳ chọn"
                value={row.note}
                onChange={(e) => handleChange(index, 'note', e.target.value)}
              />
              <datalist id="noteOptions">
                <option value="Nhập ảo" />
                <option value="Nhập thực" />
              </datalist>
